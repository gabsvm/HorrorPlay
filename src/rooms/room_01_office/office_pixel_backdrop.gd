...
func _draw_wall_and_floor() -> void:
	...
	# broad tonal bands create a painted-plaster gradient without smooth vector
	# gradients; each band remains visible as authored pixel-art value grouping.
	for y in range(0, 245, 8):
		var t := float(y) / 245.0
		var c := Color(0.065 + t * 0.025, 0.086 + t * 0.025, 0.105 + t * 0.03, 1)
...
func _draw_window() -> void:
	...
	# baked rain trails in the far layer
	var streaks := [
...
func _draw_case_board() -> void:
	...
	var papers := [Rect2(169,49,35,25), Rect2(215,46,45,31), Rect2(180,83,31,21), Rect2(224,86,29,18)]
	for r in papers:
		draw_rect(r, Color("#cbbd99"))
		draw_rect(Rect2(r.position + Vector2(2,2), Vector2(r.size.x - 4,1)), Color(0.25,0.22,0.18,0.55))
	var thread := PackedVector2Array([Vector2(184,61), Vector2(237,62), Vector2(194,94), Vector2(238,95)])
...
func _draw_bookcase() -> void:
	...
	var palette := [Color("#663c2e"), Color("#3f525b"), Color("#655333"), Color("#405946"), Color("#7a4737"), Color("#4f342b")]
	_rng.seed = 317
	for shelf_y in [84,123,162,201]:
		var x := 324
		while x < 405:
			var bw := _rng.randi_range(4,8)
			var bh := _rng.randi_range(17,27)
			var col := palette[_rng.randi_range(0,palette.size()-1)]
...
func _draw_wall_map() -> void:
	...
	var marks := PackedVector2Array([Vector2(438,91),Vector2(445,87),Vector2(453,88),Vector2(459,82),Vector2(466,85)])
...
func _draw_desk() -> void:
	...
	# folders, ashtray and coffee mug add lived-in density
...
func _draw_filing_cabinet() -> void:
	...
	# lower drawer has warmer oxidized staining
...
func _draw_door() -> void:
	...
	# cracked lintel / old building imperfection
...
func _draw_material_noise() -> void:
	# deterministic one-pixel wear marks. Static by design: no full-scene redraw
	# cost during gameplay on low-end mobile hardware.
	_rng.seed = 1926
	for i in range(180):
		var x := _rng.randi_range(8,632)
		var y := _rng.randi_range(8,224)
		var alpha := _rng.randf_range(0.05,0.12)
		draw_rect(Rect2(x,y,1,1),Color(0.60,0.66,0.66,alpha))
	for i in range(90):
		var x := _rng.randi_range(10,630)
		var y := _rng.randi_range(248,352)
