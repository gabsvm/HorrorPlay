...
func _draw_radiator() -> void:
	var origin: Vector2 = Vector2(33, 190)
	draw_rect(Rect2(origin, Vector2(88, 34)), Color("#0a0f12"))
	for i in range(8):
		var x: float = origin.x + 4 + i * 10
...
func _draw_rotary_phone() -> void:
	var c: Vector2 = Vector2(257, 207)
	draw_circle(c, 10, Color("#11171a"))
	draw_circle(c, 6, Color("#263035"))
	for angle in range(0, 360, 60):
		var r: float = deg_to_rad(float(angle))
...
func _draw_floor_clutter() -> void:
	...
	_draw_pixel_ellipse(Vector2(517, 292), Vector2(7, 3), Color(0.02, 0.03, 0.035, 0.52))
	_draw_pixel_ellipse(Vector2(541, 303), Vector2(6, 3), Color(0.02, 0.03, 0.035, 0.44))

func _draw_window_beam() -> void:
	for i in range(8):
		var alpha: float = 0.020 + float(7 - i) * 0.006
		var left: float = 39.0 + float(i) * 5.0
		var right: float = 129.0 + float(i) * 8.0
		var bottom_y: float = 284.0 + float(i) * 4.0
		draw_colored_polygon(PackedVector2Array([
